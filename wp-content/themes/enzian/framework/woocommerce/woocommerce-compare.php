...
if(!class_exists('Enzian_WooCommerce_Compare')){
    class Enzian_WooCommerce_Compare {
	    ...
	    public function new_ajax_process( $request = [] ){
		    ...
		    $return_data['compare_url'] = enzian_get_compare_url();
        ...
        public function remove_item_from_url(){
            if(isset($_GET['la_helpers_compare_remove']) && ( $product_id = absint($_GET['la_helpers_compare_remove'])) ) {
                ...
                if(($key = array_search($product_id, $product_exists)) !== false) {
                    ...
                    wc_add_notice( sprintf( esc_html__('"%1$s" has been removed from list', 'enzian'), get_the_title($product_id) ) );
                }
            }
        }
        ...
        private function add( $post_id = 0 ){
            ...
            if(in_array($post_id, $lists)){
                $response['message'] = esc_html__('Product already in list', 'enzian');
                $response['has_error'] = true;
            }
            else{
                $response['message'] = sprintf( esc_html__('"%1$s" has been added', 'enzian'), get_the_title($post_id) );
        ...
        private function remove( $post_id = 0 ){
            ...
            if(($key = array_search($post_id, $lists)) !== false) {
                $response['message'] = esc_html__('Product has been removed from list', 'enzian');
                $response['has_error'] = false;
                unset($lists[$key]);
                $lists = array_values($lists);
                $this->set_lists_for_user($lists);
                ...
                $response['message'] = esc_html__('Product does not exist in list', 'enzian');
                $response['has_error'] = true;
            }
            return $response;
        }
        ...
        public static function get_default_attributes(){
            return array(
                'image'         => esc_html__( 'Image', 'enzian' ),
                'title'         => esc_html__( 'Title', 'enzian' ),
                'add-to-cart'   => esc_html__( 'Add to cart', 'enzian' ),
                'price'         => esc_html__( 'Price', 'enzian' ),
                'sku'           => esc_html__( 'Sku', 'enzian' ),
                'description'   => esc_html__( 'Description', 'enzian' ),
                'stock'         => esc_html__( 'Availability', 'enzian' ),
                'weight'        => esc_html__( 'Weight', 'enzian' ),
                'dimensions'    => esc_html__( 'Dimensions', 'enzian' )
            );
        }
...
new Enzian_WooCommerce_Compare();