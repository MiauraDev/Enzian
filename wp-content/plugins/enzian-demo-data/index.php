<?php
/*
Plugin Name:    Enzian Package Demo Data
...
if(!function_exists('la_import_get_post_id_by_title')){
    function la_import_get_post_id_by_title( $slug, $post_type ){
        global $wpdb;
        $post_name = wp_unslash( sanitize_post_field( 'post_title', $slug, 0, 'db' ) );
        $object_type = wp_unslash( sanitize_post_field( 'post_type', $post_type, 0, 'db' ) );
        $query = "SELECT ID FROM $wpdb->posts WHERE 1=1";
        $args = array();
        if ( !empty ( $post_name ) ) {
...
class Enzian_Data_Demo_Plugin_Class{
    ...
    public static $theme_name = 'enzian';

    public static $demo_site = 'https://enzian.la-studioweb.com/';
...
add_action('plugins_loaded', function(){
    ...
    if(strpos(strtolower($theme->get_template()), 'enzian') === false){

        add_action( 'admin_notices', function(){
            printf( __( '%1$s"Enzian Package Demo Data" requires %3$s"Enzian"%4$s theme to be installed and activated. Please active %3$s"Enzian"%4$s to continue.%2$s', 'lastudio-demodata' ), '<div class="error"><p>', '</p></div>' ,'<strong>', '</strong>' );
        });
    ...
    Enzian_Data_Demo_Plugin_Class::get_instance();
}, 999);